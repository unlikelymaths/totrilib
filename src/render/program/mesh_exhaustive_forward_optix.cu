...
extern "C" __constant__ MeshExhaustiveForwardLaunchParams mesh_exhaustive_forward_launch_params;

extern "C" __global__ void __raygen__rg() {
  MeshExhaustiveForwardLaunchParams& p = mesh_exhaustive_forward_launch_params;
  ...
  float distance_0 = length(vertex_0 - scan_point) + length(vertex_0 - laser_point);
  float distance_1 = length(vertex_1 - scan_point) + length(vertex_1 - laser_point);
  float distance_2 = length(vertex_2 - scan_point) + length(vertex_2 - laser_point);
  ...
  if (bin_0 == bin_2) {
    if (bin_0 >= 0 && bin_0 < p.transient.size(1)) {
      atomicAdd(&p.transient[p.b][bin_0][s][l], alpha);
    }
  } else {
    const float alpha_ycenter = 2.0f * alpha / (cbin_2 - cbin_0);
    DrawTrapezoid(cbin_0,           0.f, cbin_1, alpha_ycenter, s, l, p);
    DrawTrapezoid(cbin_1, alpha_ycenter, cbin_2,           0.f, s, l, p);
  }
}
